...
/*   Updated: 2019/08/10 16:35:29 by sbosmer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		draw_wall(t_data *d, const int x, const int height, t_raycast ray)
{
	...
	if (ray.type == -1)
		...
		tex = d->tex_pool[(d->ctrl.zharko ? 8 : ray.type - 1)];
	ray.hit.x = ray.hit.x - floorf(ray.hit.x + 0.5);
	ray.hit.y = ray.hit.y - floorf(ray.hit.y + 0.5);
	tex_x = (fabs(ray.hit.x) > fabs(ray.hit.y)
		? ray.hit.x : ray.hit.y) * (float)tex.width;
	(y = (TEX_HEIGHT - height) / 2) > -99 && tex_x < 0 ? tex_x += tex.width : 1;
	...
	while (++qt < ft_clamp(y + height, 0, TEX_HEIGHT))
	{
		...
		SDL_SetRenderDrawColor(d->sdl.ren, tex.data[id * 3],
			tex.data[id * 3 + 1], tex.data[id * 3 + 2], 0xff);
...
void		render_walls(t_data *d)
{
	...
	{
		ray = d->rend.rays[x];
		height = ray.dist * cos(angle_delta * x - FIELD_OF_VIEW / 2);
